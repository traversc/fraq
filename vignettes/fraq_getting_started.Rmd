...
- `.mem` - In-memory `.fraq` (read/write). Lifetime is limited to the current
  ...
  they are not normalized or expanded.
...
- Return a named list of data frames to write reads. Each list name is an
  output path or `.mem` key, and each data frame must contain `name`, `seq`,
  and `qual` columns. Output paths returned by an R kernel are normalized by
  the R wrapper; `.mem` keys are used exactly as returned.
